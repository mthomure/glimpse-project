...
cdef extern from "src/filters.h" nogil:
  ...
  void CLocalMax(ArrayRef3D[float]& input, int kheight, int kwidth, int scaling,
      ArrayRef3D[float]& output, BitsetArrayRef3D& max_coords) except +
...
def ContrastEnhance(np.ndarray[act_t, ndim = 2] in_data not None, int kheight,
  ...
  if out_data == None:
    ...
    oheight, owidth, multi_kwidth, lpad = MaxOutputDimensions(kheight, kwidth,
        1, (in_data.shape[0], in_data.shape[1]))
...
def _Prepare3dFilterArgs(np.ndarray[act_t, ndim = 3] in_data not None,
  ...
  if out_data == None:
    ...
    if kheight != kwidth:
      raise ValueError("Kernel must be square (spatially)")
    oheight, owidth, multi_kwidth, lpad = MaxOutputDimensions(kheight, kwidth,
        scaling, (in_data.shape[1], in_data.shape[2]))
...
def LocalMax(np.ndarray[act_t, ndim = 3] idata not None,
