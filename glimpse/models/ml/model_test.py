...
class TestModel(unittest.TestCase):
  ...
  def testBuildS1(self):
    result = self.BuildLayerFromPath(Model.Layer.S1, list)
  ...
  def testBuildC1(self):
    result = self.BuildLayerFromPath(Model.Layer.C1, list)
    self.assertEqual(len(result), self.NUM_SCALES)
    for scale in result:
      self.assertTrue(isinstance(scale, np.ndarray))
      self.assertEqual(scale.ndim, 3)
  ...
  def testBuildS2(self):
    result = self.BuildLayerFromPath(Model.Layer.S2, list)
    self.assertEqual(len(result), self.NUM_SCALES)
    for scale in result:
      self.assertTrue(isinstance(scale, np.ndarray))
      self.assertEqual(scale.ndim, 3)
